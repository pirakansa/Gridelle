...
- **シート管理**: プルダウンでシート切替、テキストボックスでシート名変更（Enterまたはフォーカスアウトで確定）、「シートを追加」で新規シートを作成します。
...
- レガシー形式: ルート配列に `name` や `rows` を持たないオブジェクトのみが並んでいる場合は、単一シート（名前は `Sheet 1`）として読み込みます。
...
上記のように `rows` を持たない配列は、読み込み時に `Sheet 1` という単一シートに変換されます。
