...
- **行操作**: 行番号ボタンで行全体を選択、Shift+クリックで複数行を連続選択。「行を追加」で末尾に空行を追加し、「選択行を削除」で選択範囲の行をまとめて削除します。
- **列操作**: 列名横の矢印で左右へ並べ替え。「列を追加」で自動命名された列を追加できます（既存列数に応じて `column_番号` を採番）。貼り付けやフィルでも不足する列は自動生成されます。
- **一括入力**: 「選択セルへ一括入力」に値を入れて「一括入力する」を押すと、選択範囲の全セルをまとめて更新します。
- **セルのスタイル**: メニュー「選択」タブ内の「セルのスタイル」で文字色・背景色を指定できます。設定した色はYAML入出力時に `color`・`bgColor` として保存されます。
...
- セル表現: 各セルは単純な文字列か、`{ value, color?, bgColor? }` 形式のオブジェクトとして保存されます。`color` と `bgColor` は省略可能で、空文字や空白のみの値は自動的に無視されます。
