...
- Validation commands (`npm test`, `npm run type-check`, `npm run lint`, `npm run build`)
...
- `npm test && npm run type-check && npm run lint && npm run build` succeeds when behavior changed.
