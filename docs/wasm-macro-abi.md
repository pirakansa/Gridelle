...
行・列いずれの指定でも、特殊文字列 `"@self"` を使うと「関数を設定したセルと同じ行（または列）」を指すことができます。

- `rows: '@self'` : 関数を評価しているセルと同じ行のみを対象にする。
- `key: '@self'` / `keys: ['@self', 'effort']` : 自身の列（+任意の列）を対象にする。
- `columns: { start: '@self', end: '@self' }` : 行方向集計時に自身の列だけを扱う。
- `cells: [{ row: '@self', key: '@self' }]` : 明示的なセル指定で自身のセルを再参照する。

`@self` は `columnIndex` や `rows: [1, '@self']` のような複合指定にも利用可能です。コピー & ペーストでセルが移動した場合でも、`@self` を使った参照は自動的に新しい位置へ追従します。
